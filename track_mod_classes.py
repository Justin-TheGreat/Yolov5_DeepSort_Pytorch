...
# def Speedtrack(cen_x,cen_y, frame_idx ,identities=None, offset=(0, 0)):
#     how_many_frame_will_detect = 5
#     if (frame_idx >= how_many_frame_will_detect + 1) and (frame_idx % how_many_frame_will_detect = 0):
#             np_store[i][2] = np_store[i][4]
#             np_store[i][3] = np_store[i][5]
#             np_store[i][4] = cen_x
#             np_store[i][5] = cen_y
#
#     return velocity, distance

  # cv2.line(img, (int((x1+x2)/2), int(y1 + (y2 - y1) * 0.1)) , (int((x1+x2)/2),int(y1+(y2-y1)*0.8)), (0,0,255), 3)
  # cv2.circle(img, (int((x1+x2)/2),int(y1 + (y2 - y1) * 0.85)), 3, (0,0,255), -1)
...
def draw_boxes(img, bbox, frame_idx, myclasses ,identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
      ...
      if (frame_idx >= how_many_frame_will_detect + 1) and (frame_idx % how_many_frame_will_detect == 0):
          ...
          np_store[i][4] = cen_x
          np_store[i][5] = cen_y
      velocity = int(np.sqrt(((np_store[i][5]-np_store[i][3])**2 + (np_store[i][4] - np_store[i][2])**2))/(1/30*how_many_frame_will_detect))

      # box text and bar
      color = compute_color_for_labels(id)
      label = '{}{:d} {} {} km/hr x:{} y:{} class:{}'.format("", id, frame_idx, velocity, cen_x, cen_y,myclasses)
